...
def get_profile(id: str, db: Session = Depends(get_db)):
    single_profile = db.query(profile.Profile).filter(
        profile.Profile.user_id == id).first()
...
def update_profile(id: str, request: ProfileSchema, db: Session = Depends(get_db)):
    ...
    single_profile.update({'user_id': request.user_id, 'first_name': request.first_name, 'last_name': request.last_name, 
                           'mobile_number': request.mobile_number, 'photo': request.photo})
    db.commit()
